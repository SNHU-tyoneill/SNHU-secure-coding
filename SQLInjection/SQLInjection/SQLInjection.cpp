...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Find the position of " where " in the SQL statement
    size_t where_pos = sql_lower.find(" where ");
    if (where_pos != std::string::npos) {
        // Extract the WHERE clause
        std::string where_clause = sql_lower.substr(where_pos + 7); // 7 is the length

        // Check for suspucious patters in the WHERE clause
        // Looking for ' or ', ' and ', ';', '--', '/*', '*/'

        std::vector<std::string> sql_injection_patterns = { " or ", " and ", ";", "--", "/*", "*/", " union ", " select " };

        bool injection_detected = false;
        for (const auto& pattern : sql_injection_patterns) {
            if (where_clause.find(pattern) != std::string::npos) {
        ...
        if (injection_detected) {
            std::cout << "Error: Suspected SQL Injection detected in WHERE clause. Query execution aborted." << std::endl;
            return false;
        }
    }

    // Proceed with the execution of the query if no injection is detected.
    char* error_message;
